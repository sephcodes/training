...
def load_dataset(*, file_name: str) -> pd.DataFrame:
    dataframe = pd.read_csv(Path(f"{DATASET_DIR}/{file_name}"))
    dataframe = dataframe.rename(columns=config.model_config.variables_to_rename)
    transformed = transform_dataframe(dataframe=dataframe)
...
def transform_dataframe(*, dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # get first cabin if more than one
    data["cabin"] = data["cabin"].apply(get_first_cabin)

    # get just title from name column
    data["title"] = data["name"].apply(get_title)

    # convert numerical columns to actual numeric type
    data["fare"] = data["fare"].astype("float")
    data["age"] = data["age"].astype("float")

    # drop unnecessary columns
    data = data.drop(config.model_config.unused_variables, axis=1)
